...
void TextColorizer::writeSettings(const QString &basePath)
{
	Settings s(basePath);
	int idx = 0;
...
void TextColorizer::readSettings(const QString &basePath)
{
	_coloredTextParts.clear();
	Settings s(basePath);
	foreach(QString colStr, s.childGroups("textColorizer")) {		
		int idx = colStr.toInt();
		TextColorize colorize;
		colorize.text = s.textColorizer(idx).text();
		colorize.font = s.textColorizer(idx).font();
		colorize.useRegex = s.textColorizer(idx).useRegex();
		colorize.wordOnly = s.textColorizer(idx).wordOnly();
		colorize.wholeRow = s.textColorizer(idx).wholeRow();
		colorize.caseSensitive = s.textColorizer(idx).caseSensitive();
		colorize.foregroundColor = s.textColorizer(idx).foregroundColor();
		colorize.backgroundColor = s.textColorizer(idx).backgroundColor();
		_coloredTextParts.push_back(colorize);
	}
}