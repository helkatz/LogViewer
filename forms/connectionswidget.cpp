...
void ConnectionsWidget::on_cbName_currentTextChanged(const QString &arg1)
{
}

void ConnectionsWidget::on_cbName_currentIndexChanged(int index)
{
    QString name = ui->cbName->currentText();
    if(name.length() == 0)
        return;
...
void ConnectionsWidget::on_btnTest_clicked()
{
    ...
    QSqlDatabase db = Utils::getDatabase(conName,
